...
/*
  ...
  Gary Jennejohn    : gj@denx.de / Kernel 2.4.18
		    : modified for the S3C2400 which is	not really an
		    : ISA configuration	but rather directly mapped into
		    : the memory space.	These modifications are	based on
		    : the 2.2.13 code from Samsung.
		    
  ghcstop	    : airdiver@korea.com / kernel 2.4.18
		    : modified for the S3C2410,	smdk2410 
 ...
 */
#define	DEBUGGING	1     // ghcstop fix
...
#include <asm/hardware.h>
#define	readIOPort(offset) *((volatile unsigned	short *)(offset))
#define	writeIOPort(data, offset) *((volatile unsigned short *)(offset)) \
				  = (unsigned short)(data)
//static unsigned int netcard_portlist[] __initdata = {	0xe7000300, 0 };
...
static irqreturn_t net_interrupt(int irq, void	*dev_id, struct	pt_regs	*regs); // ghcstop fix
...
{
	int i;
    int	base_addr = dev	? dev->base_addr : 0; // ghcstop
...
#ifdef CONFIG_ARCH_S3C2440
	//printk("cs89x0 probing...\n");
	printk("cs89x0:cs89x0_probe(0x%x)\n", base_addr);    
#endif
	for (i = 0; netcard_portlist[i]; i++) {
#ifdef CONFIG_ARCH_S3C2440
		if (check_region(netcard_portlist[i], NETCARD_IO_EXTENT))
			continue;
#endif
...
{
#ifdef CONFIG_ARCH_S3C2440
	writeIOPort(portno, dev->base_addr + ADD_PORT);
	return readIOPort(dev->base_addr + DATA_PORT);
#else
...
#ifdef CONFIG_ARCH_S3C2440
	writeIOPort(portno, dev->base_addr + ADD_PORT);
	writeIOPort(value, dev->base_addr + DATA_PORT);
#else
	outw(portno, dev->base_addr + ADD_PORT);
...
#else
	return readIOPort(dev->base_addr + portno);
#endif
}
...
#else
	writeIOPort(value, dev->base_addr + portno);
#endif
}
...
#ifdef CONFIG_ARCH_S3C2440 // ghcstop fix
/*
	BWSCON &= ~(BWSCON_ST3 | BWSCON_WS3 | BWSCON_DW3);
	BWSCON |=  (BWSCON_ST3 | BWSCON_WS3 | BWSCON_DW3_16);
	#if 0
	BANKCON3= BANKCON_Tacs0 | BANKCON_Tcos4 | BANKCON_Tacc14 |
		BANKCON_Toch1 | BANKCON_Tcah0 | BANKCON_Tacp6 | BANKCON_PMC1;
	#else
	BANKCON3= BANKCON_Tacs0 | BANKCON_Tcos4 | BANKCON_Tacc14 |
		BANKCON_Toch1 | BANKCON_Tcah4 | BANKCON_Tacp6 | BANKCON_PMC16;
	#endif
	
	set_external_irq(IRQ_EINT9, EXT_RISING_EDGE, GPIO_PULLUP_DIS);
*/	
...
printk("4\n");
	...
	if ((readIOPort(ioaddr + ADD_PORT) & ADD_MASK) != ADD_SIG) {
		retval = -ENODEV;
		goto out1;
	}
	if (readreg(dev, PP_ChipID) != CHIP_EISA_ID_SIG) {
		printk("return !!, product ID error\n");
		retval = -ENODEV;
		goto out1;
	}
#endif
	...
	//dev->irq = 5;
	dev->irq = IRQ_EINT9;
	/* this	should be handled by ethaddr_setup() */
	/* TEST	*/
	retval = 0;
	for (i = 0; i <	6; i++)
		retval += my_ethaddr[i];
	/* ethaddr not set in armboot ?	*/
	if (retval == 0)
	{
		dev->dev_addr[0] = 0x00;
		dev->dev_addr[1] = 0x11;
...
#ifdef CONFIG_ARCH_S3C2440
	writereg(dev, PP_BusCTL, 0); /*	disabled interrupt, I/O	mode */
	/* from	TBEL, SA_INTERRUPT, or 0 */
	if (request_irq(dev->irq, net_interrupt, SA_SHIRQ, "cs8900a", dev)) {
		printk("request	IRQ is busy\n");
		ret = -EAGAIN;
		goto bad_out;
	}
	/* we set IRQ0 for internal IRQ	on CS8900, by chc */
...
static int net_send_packet(struct sk_buff *skb,	struct net_device *dev)
{
...
#ifdef CONFIG_ARCH_S3C2440
	/* 0x0009 + 0x00c0: marked by by chc */
	/* 0x0009 is the reserved bits */
	/* 0x00c0 means	start transmission after the entire frame is */
	/* in the CS8900A */
	lp->send_cmd = 0x00c9;
#endif
	...
	for (i = 0; i <	(skb->len >> 1); i++) {
		tmp = (skb->data[2*i] &	0xff) |	((skb->data[2*i	+ 1] <<	8) & 0xff00);
		writeIOPort(tmp, dev->base_addr	+ TX_FRAME_PORT);
	}
	if (skb->len & 1) {
		tmp = skb->data[skb->len - 1] &	0xff;
		writeIOPort(tmp, dev->base_addr	+ TX_FRAME_PORT);
	}
#endif
	spin_unlock_irq(&lp->lock);
...
static irqreturn_t net_interrupt(int irq, void	*dev_id, struct	pt_regs	* regs) // ghcstop fix
{
	...
	return IRQ_RETVAL(handled); // ghcstop fix
}
...
net_rx(struct net_device *dev)
{
	...
	unsigned short tmp;
	int i;
	status = readIOPort(ioaddr + RX_FRAME_PORT);
	length = readIOPort(ioaddr + RX_FRAME_PORT);
#endif
	...
	for (i = 0; i <	(length	>> 1); i++) {
		tmp = readIOPort(ioaddr	+ RX_FRAME_PORT);
		skb->data[2*i] = (unsigned char)(tmp & 0xff);
		skb->data[2*i+1] = (unsigned char)(tmp >> 8 & 0xff);
	}
	if (length & 1)	{
		tmp = readIOPort(ioaddr	+ RX_FRAME_PORT);
		skb->data[length - 1] =	(unsigned char)(tmp & 0xff);
	}
#endif
...
#else
		writeIOPort(PP_ChipID, dev_cs89x0.base_addr + ADD_PORT);
#endif
		kfree(dev_cs89x0.priv);
