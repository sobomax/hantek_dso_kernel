...
// ghcstop: lcd device driver로의 platform data(user define data)로 건네지는 넘은 s3c2410fb_mach_info 이다.
static struct s3c2410fb_mach_info s3c2410fb_info;

// ghcstop
// 이 함수를 불러줘야 하는 군. platform_data를 set하는 함수이다.
// 이 함수는 mach-aesop2440.c의 aesop2440_init()에서 호출된다.
